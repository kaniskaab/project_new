...
const Patient = () => {
  ...
  return (
    <>
      <Header />
      <div
        className="h-auto w-screen bg-no-repeat bg-cover overflow-hidden"
        style={{ backgroundImage: `url(${img})` }}
      >
        <div class="grid grid-rows-6 grid-cols-12  grid-flow-col gap-1">
          <div class="row-span-6 col-span-3 ">
            <Sidebar memberdata={data} id={id} />
          </div>
          <motion.div
            class="col-span-9 row-span-6 bg-transparent flex flex-col items-center h-full w-full overflow-hidden"
          ...
          >
            ...
            <div className="grid grid-cols-2 grid-row-2 grid-flow-col text-xl ">
              <motion.div
                className=" flex flex-col row-span-1 col-span-1 m-3 p-5 items-center b hover:bg-blue-800 hover:bg-opacity-50 rounded-2xl "
                animate={{ scale: 1.1, marginLeft: "2px", opacity: 1 }}
                transition={{ ease: "easeOut", duration: 1, type: "tween" }}
                initial={{ scale: 0, opacity: 0.5 }}
              >
                <h1 className="text-3xl font-dance underline">
                  General Information
                </h1>
              ...
              <motion.div
                className="flex flex-col m-3 p-5 items-center  rounded-2xl hover:bg-blue-800 hover:bg-opacity-50 "
                animate={{ scale: 1.1, marginLeft: "2px", opacity: 1 }}
                transition={{ ease: "easeOut", duration: 1, type: "tween" }}
                initial={{ scale: 0, opacity: 0.5 }}
              >
